...
const { validateApplication, enrichApplication, renderInternalEmail } = require('./_lib/supplier-programme');
...
async function postJson(url, payload, headers = {}) {
  ...
  try {
    ...
    console.error('Supplier Programme delivery error:', error.message);
...
module.exports = async function handler(req, res) {
  ...
  const makeUrl = process.env.ORIGIN_MAKE_WEBHOOK_URL || process.env.MAKE_WEBHOOK_URL;
  const makeApiKey = process.env.ORIGIN_MAKE_WEBHOOK_API_KEY;
  const makeHeaders = makeApiKey ? { 'X-Make-Apikey': makeApiKey } : {};
  const make = await postJson(makeUrl, { formType: 'supplier-programme', ...record }, makeHeaders);
  const makeRecorded = make.ok && make.body?.ok === true && make.body?.recorded === true;

  if (makeRecorded) {
    return res.status(200).json({ success: true, applicationId: record.applicationId, responseDueAt: record.responseDueAt });
  }
