/**
 * "Prism" WebGL background — vanilla port of the reactbits.dev Prism
 * component (React + ogl) for pages that aren't React-bundled.
 *
 * Reads config from data-* attributes on its container element so the
 * same file can be dropped on other pages by giving them a matching
 * container id. Loads ogl at runtime from a CDN ESM build (no npm
 * dependency, matching js/rc-gallery.js). If WebGL or the ogl import
 * fails, it warns and leaves the container empty — the page's own CSS
 * background remains the fallback visual.
 */
(async function () {
  'use strict';

  const container = document.getElementById('demo-prism-bg');
  if (!container) return;
  ...
  function readConfig() {
    ...
    return {
      ...
      animationType: str('animationType', DEFAULTS.animationType),
      glow: num('glow', DEFAULTS.glow),
      offsetX: num('offsetX', DEFAULTS.offsetX),
      offsetY: num('offsetY', DEFAULTS.offsetY),
      noise: num('noise', DEFAULTS.noise),
      transparent: bool('transparent', DEFAULTS.transparent),
      scale: num('scale', DEFAULTS.scale),
      hueShift: num('hueShift', DEFAULTS.hueShift),
      colorFrequency: num('colorFrequency', DEFAULTS.colorFrequency),
  ...
  let OGL;
  try {
    OGL = await import(/* webpackIgnore: true */ config.oglUrl);
  } catch (err) {
    console.warn('prism-bg: could not load ogl from CDN, skipping background.', err);
    return;
  }
  ...
  const TS = prefersReducedMotion ? 0 : Math.max(0, config.timeScale || 1);
  const HOVSTR = Math.max(0, config.hoverStrength || 1);
  const INERT = Math.max(0, Math.min(1, config.inertia || 0.12));
  ...
  if (animationType === 'hover') {
    const onPointerMove = e => { onMove(e); startRAF(); };
