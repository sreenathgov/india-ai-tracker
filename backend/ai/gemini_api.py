...
class GeminiProcessor:
    ...
    def _build_batch_prompt(self, articles: List[Dict[str, str]]) -> str:
        ...
        prompt_parts.extend([
            ...
            "   - DEFAULT: ['IN'] for national/All India - use this unless state is CLEARLY the subject",
            "   - Assign specific state ONLY if article is SUBSTANTIVELY ABOUT that state:",
            "     * State government announces/implements AI policy/initiative/scheme",
            "     * AI facility/hub/office physically opening IN that state",
            "     * AI event physically held IN that state (not online events)",
            "     * Investment/deployment explicitly TIED to that state",
            "   - DO NOT assign state based on:",
            "     * Company HQ (Zoho product launch = 'IN', not 'TN')",
            "     * Startup base (Sarvam AI funding = 'IN', not 'TG')",
            "     * News source location or datelines",
